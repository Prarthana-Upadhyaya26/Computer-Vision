import numpy as np
import cv2
from ultralytics import YOLO

model = YOLO('yolov8m.pt')

vertices1 = np.array([(200, 0), (0, 0), (0, 350), (200, 350)], dtype=np.int32)
vertices2 = np.array([(300, 0), (220, 0), (220, 350), (300, 350)], dtype=np.int32)

# Define the vertical range for the slice and lane threshold
x1 = 300
lane_threshold = 210

# Define the positions for the text annotations on the image
text_position_left_lane = (10, 50)
text_position_right_lane = (450, 50)

# Define font, scale, and colors for the annotations
font = cv2.FONT_HERSHEY_SIMPLEX
font_scale = 0.7
font_color = (0, 0, 255)  # White color for text
background_color = (0, 0, 255)  # Red background for text

# Open the video
cap = cv2.VideoCapture('main_videos/original/shopping_girl_ketchup.mp4')

# Define the codec and create VideoWriter object
fourcc = cv2.VideoWriter_fourcc(*'XVID')
out = cv2.VideoWriter('main_videos/predicted/shopping_girl_ketchup.mp4', fourcc, 20.0, (int(cap.get(3)), int(cap.get(4))))
heavy_traffic_threshold = 0
while cap.isOpened():
    # Capture frame-by-frame
    ret, frame = cap.read()
    if ret:
        # Create a copy of the original frame to modify
        detection_frame = frame.copy()

        # Black out the regions outside the specified vertical range
        detection_frame[:, x1:] = 0  # Black out from top to x1

        # Perform inference on the modified frame
        results = model.predict(detection_frame, imgsz=640, conf=0.4)
        processed_frame = results[0].plot(line_width=1)

        # Restore the original top and bottom parts of the frame
        processed_frame[:, x1:] = frame[:, x1:].copy()

        # Draw the quadrilaterals on the processed frame
        cv2.polylines(processed_frame, [vertices1], isClosed=True, color=(0, 255, 0), thickness=2)
        cv2.polylines(processed_frame, [vertices2], isClosed=True, color=(255, 0, 0), thickness=2)

        # Retrieve the bounding boxes from the results
        bounding_boxes = results[0].boxes

        # Initialize counters for vehicles in each lane
        onself = 0
        offself = 0

        # Loop through each bounding box to count vehicles in each lane
        for box in bounding_boxes.xyxy:
            # Check if the vehicle is in the left lane based on the x-coordinate of the bounding box
            if box[0] < lane_threshold:
                onself += 1
            else:
                offself += 1

        # Add the vehicle count text on top of the rectangle for the left lane
        cv2.putText(processed_frame, f'items on shelf: {onself}', text_position_left_lane,
                    font, font_scale, font_color, 2, cv2.LINE_AA)

        # Add the vehicle count text on top of the rectangle for the right lane
        cv2.putText(processed_frame, f'items picked: {offself}', text_position_right_lane,
                    font, font_scale, font_color, 2, cv2.LINE_AA)

        # Write the processed frame to the output video
        out.write(processed_frame)

        #  running this code on a local machine to view the real-time processing
        cv2.imshow('Real-time Analysis', processed_frame)
        if cv2.waitKey(1) & 0xFF == ord('q'):  # Press Q on keyboard to exit the loop
            break
    else:
        break

    # Release the video capture and video write objects
cap.release()
out.release()